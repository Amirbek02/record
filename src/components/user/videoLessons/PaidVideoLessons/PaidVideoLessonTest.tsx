...
const PaidVideoLessonTest = () => {
  const [testData] = useState<TestSession>({
    id: 223,
    department: 1,
    subject: 'Математика',
  ...
  useEffect(() => {
    ...
    const answers: TestAnswers = {
      ...
      startTime: JSON.parse(localStorage.getItem(`test-session-${testData.id}`) || '{}').startTime,
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto p-4">
      ...
      <div className="flex flex-col gap-4 md:gap-0 md:flex-row justify-center items-center md:justify-between">
        <h1 className="text-xl md:text-2xl font-semibold ">{testData.subject}</h1>
        <div className="flex flex-col items-center mb-6 max-w-[200px] ">
          <Clock size={82} strokeWidth={1.25} />
          <span className="text-xl md:text-2xl font-mono">{formatTime(timeLeft)}</span>
        </div>
      </div>
        ...
        <Card className="flex-1 border-none shadow-none">
          <CardContent className="p-3 md:p-6">
            <div className="flex justify-between">
              <h1 className="text-xl text-gray-700 md:text-2xl font-semibold">Тапшырма</h1>
            ...
            <div className="flex flex-col lg:flex-row gap-2 md:gap-4">
              <div className="flex-1">
                <div className="whitespace-pre-line mb-4 md:mb-6 text-sm md:text-xl">
                  <p>
                    Эгерде <span className="font-bold underline text-black">А КОЛОНКАСЫНДАГЫ</span>{' '}
                    чоңдук чоң болсо, анда (А);
                  </p>
                  <p>
                    Эгерде <span className="font-bold underline text-black">Б КОЛОНКАСЫНДАГЫ</span>{' '}
                    чоңдук чоң болсо, анда (Б);
                  </p>
                  <p>Эгерде эки чоңдук барабар болсо, анда (В);</p>
                  <p>
                    Эгерде бар болгон маалымат аркылуу бул чоңдуктардын кайсынысы чоң экендигин, же
                    алардын барабар экендигин аныктоо мүмкүн болбосо, анда (Г)
                  </p>
                </div>
