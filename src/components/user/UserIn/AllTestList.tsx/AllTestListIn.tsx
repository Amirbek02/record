...
const AllTestListIn = () => {
	const { data, loading, error, getSub } = userInTests();
	...
	return (
		...
		// 				{mainTests.map((testi) => (
		// 					<Link key={testi.id} href={`/all-tests/${testi.id}`}>
		// 						<TestCardNoSign
		// 							testTitle={
		// 								testi.subject_category_name || "Без названия"
		// 							}
		...
		// </div>
		<>
			{/* <TestListIn tests={data} isMainTest /> */}
			<TestListIn tests={data} />
		</>
	);
};
