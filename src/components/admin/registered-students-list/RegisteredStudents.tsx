...
import RegisteredStudentsTest from './RegisteredStudentsTest';
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      <div className="flex items-center  bg-[#F6EEFF] p-3 rounded-lg my-4 gap-32">
        <h1 className="text-[24px]">Катталган окуучулардын тизмеси</h1>
      ...
      <Accordion type="multiple"  className="space-y-2">
        {students.map((student) => (
          ...
          >
            ...
            <AccordionContent className="p-2 bg-white flex justify-end rounded-lg">
              <RegisteredStudentsTest />
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
      <div className='flex justify-end gap-6 items-center m-10'>
        <span className='flex gap-2'><X/>Cancel</span>
        <button className='w-[119px] h-[56px] bg-[#2D7653] rounded-[28px] text-white'>Save</button>
      </div>
    </div>
  );
};
